...
class TestQLibDataProviderDaily(TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        # cls.path = str(Path(str(os.getenv("HOME}"))) / Path('.qlib/qlib_data/cn_data'))  # Use QLib cn data for testing
        cls.data_provider = QLibDataProvider(tickers=cls.tickers, field_to_price_field_dict=cls.field_to_price_field_dict,
                                            fields=cls.fields, start_date=cls.start_date, end_date=cls.end_date,
                                            frequency=Frequency.DAILY)

    def test_cached_parameters(self):
        self.assertCountEqual(self.data_provider.cached_tickers,
                              {self.tickers[i]: self.tickers[i] for i in range(len(self.tickers))})
    ...
    def test_get_price_single_ticker_single_field_single_date(self):
        ...
        self.assertEqual(prices, 5417.64794921875)
